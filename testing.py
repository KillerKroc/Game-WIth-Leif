...
#modify combat for enemy to match player
#add visuals for embarked stuff and bears
#add option to switch vs embark
#add player variable to pass into splash rather than using player turn
...
def update(displayUnit):
    ...
    for card in board:
        ...
        if(card >= 1):
            ...
            #check code below it could be whack
            #if(card == active):
                ...
                #t(dmg, screen, 150 * ((board.index(card) - move) % 7) + 445 + 10, 220 * (board.index(card) // 7) + 61 - 30)
...
def boardInput(index):
    ...
    if(selectBoard[index] <= -3):
        ...
        if(board.count(select) == 0):
            ...
            #place unit
            board[index] = select
        #if moving a guy into an open vehicle
        elif board[index] >= 1 and battleDict[select][1][4] == 0 and battleDict[board[index]][1][4] >= 1 and battleDict[board[index]][2][1] ==0:
        ...
        #if moving an open vehicle into a guy
        elif board[index] >= 1 and battleDict[board[index]][1][4] == 1 and battleDict[select][1][4] >= 1 and battleDict[select][2][1] == 0:
            if(index % 7 == 0):
                board[index] = -1
            elif(index % 7 == 6):
                board[index] = -2
    ...
    if (playerTurn == True):
        ...
        for unit in board:
            if(unit >= 1 and unit <= 15):
                ...
                embarkedAttacks = 0
                embarkedRanging = 1
                if battleDict[unit][2][1] >= 1:
                    embarkedAttacks = battleDict[battleDict[unit][2][1]][0][3]
                while True:
                    if battleDict[unit][2][1] >= 1 and (attacks == 0 or ranging == battleDict[unit][0][4] + 1) and (move == battleDict[unit][0][2] or board[board.index(unit) - (move + 1)] != 0) and (embarkedAttacks == 0 or embarkedRanging == battleDict[battleDict[unit][2][1]][0][4] + 1):
                        break
                    elif(attacks == 0 or ranging == battleDict[unit][0][4] + 1) and (move == battleDict[unit][0][2] or board[board.index(unit) - (move + 1)] <= -1 or board[board.index(unit) - (move + 1)] >= 16 or (battleDict[unit][1][5] >= 1 and battleDict[board[board.index(unit) - (move + 1)]][1][5] >= 1) or (battleDict[unit][1][5] == 0 and battleDict[board[board.index(unit) - (move + 1)]][1][5] == 0) or battleDict[board[board.index(unit) - (move + 1)]][2][1] != 0 or battleDict[unit][2][1] != 0):
                        break
                    elif embarkedAttacks >= 1 and embarkedRanging <= battleDict[battleDict[unit][2][1]][0][4]:
                        target = board[board.index(unit) - (embarkedRanging + move)]
                        if (target == -2):
                            enemyHp = enemyHp - battleDict[battleDict[unit][2][1]][0][0]
                            #check victory
                            embarkedAttacks = embarkedAttacks - 1
                        elif (target >= 16):
                            battleDict[target][0][1] = battleDict[target][0][1] - battleDict[battleDict[unit][2][1]][0][0]
                            battleDict[target][2][0] = battleDict[battleDict[unit][2][1]][1][0]
                            processFatality(damagedUnit=target, player=False)
                            if battleDict[unit][1][3] >= 1:
                                splash(unit = battleDict[unit][2][1], target = target)
                            embarkedAttacks = embarkedAttacks - 1
                        else:
                            embarkedRanging = embarkedRanging + 1
                    elif attacks >= 1 and ranging <= battleDict[unit][0][4]:
                        ...
                        if (target == -2):
                            ...
                            ranging = ranging + 1
                    elif board[board.index(unit) - (move + 1)] >= 1 and board[board.index(unit) - (move + 1)] <= 15:
                        if battleDict[unit][1][5] >= 1:
                            if (board.index(unit) - (move + 1)) % 7 == 0:
                                board[board.index(unit) - (move + 1)] = -1
                            elif (board.index(unit) - (move + 1)) % 7 == 6:
                                board[board.index(unit) - (move + 1)] = -2
                            else:
                                board[board.index(unit) - (move + 1)] = 0
                            battleDict[unit][2][1] = board[board.index(unit) - (move + 1)]
                            move = move + 1
                            break
                        else:
                            if (board.index(unit) - move) % 7 == 0:
                                board[board.index(unit) - move] = -1
                            elif (board.index(unit) - move) % 7 == 6:
                                board[board.index(unit) - move] = -2
                            else:
                                board[board.index(unit) - (move + 1)] = 0
                            battleDict[board[board.index(unit) - (move + 1)]][2][1] = unit
                            break
                    else:
                        move = move + 1
                        ranging = 1
        ...
        for unit in board:
            if(unit >= 16):
                ...
                while True:
                    if battleDict[unit][2][1] >= 1 and (attacks == 0 or ranging == battleDict[unit][0][4] + 1) and (move == battleDict[unit][0][2] or board[board.index(unit) - (move + 1)] != 0) and (embarkedAttacks == 0 or embarkedRanging == battleDict[battleDict[unit][2][1]][0][4] + 1):
                        break
                    elif (attacks == 0 or ranging == battleDict[unit][0][4] + 1) and (move == battleDict[unit][0][2] or board[board.index(unit) - (move + 1)] <= -1 or (board[board.index(unit) - (move + 1)] >= 1 and board[board.index(unit) - (move + 1)] <= 15) or (battleDict[unit][1][5] >= 1 and battleDict[board[board.index(unit) - (move + 1)]][1][5] >= 1) or (battleDict[unit][1][5] == 0 and battleDict[board[board.index(unit) - (move + 1)]][1][5] == 0) or battleDict[board[board.index(unit) - (move + 1)]][2][1] != 0 or battleDict[unit][2][1] != 0):
                        break
                    elif embarkedAttacks >= 1 and embarkedRanging <= battleDict[battleDict[unit][2][1]][0][4]:
                        target = board[board.index(unit) - (embarkedRanging + move)]
                        if (target == -2):
                            ...
                            battleDict[target][0][1] = battleDict[target][0][1] - battleDict[battleDict[unit][2][1]][0][0]
                            battleDict[target][2][0] = battleDict[battleDict[unit][2][1]][1][0]
                            processFatality(damagedUnit=target, player=True)
                            if battleDict[unit][1][3] >= 1:
                                splash(unit = battleDict[unit][2][1], target = target)
                            embarkedAttacks = embarkedAttacks - 1
                        else:
                        ...
                        if (target == -2):
                            ...
                            ranging = ranging + 1
                    elif board[board.index(unit) - (move + 1)] >= 16:
                        if battleDict[unit][1][5] >= 1:
                            if (board.index(unit) - (move + 1)) % 7 == 0:
                                board[board.index(unit) - (move + 1)] = -1
                            elif (board.index(unit) - (move + 1)) % 7 == 6:
